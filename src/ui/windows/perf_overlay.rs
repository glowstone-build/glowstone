...
use crate::renderer::{PassTimings, gpu_timer};
...
pub fn perf_overlay_window(
    ...
    let ink = theme::ink(!ctx.global_style().visuals.dark_mode);
        ...
        .show(ctx, |ui| {
            {
                ...
                if timings.gpu_valid {
                    let full = timings.passes.iter().cloned().fold(2.0f32, f32::max); // bar full-scale = busiest pass (≥2ms)
                    for (i, (name, _sub)) in gpu_timer::BAR_LABELS.iter().enumerate() {
                        let ms = timings.passes[i];
                        ui.horizontal(|ui| {
                            ui.add_sized(
                                [82.0, 13.0],
                                egui::Label::new(RichText::new(*name).small().monospace().color(
                                    if ms <= 0.001 {
                                        ink.muted
                                    } else {
                                        ink.secondary
                            ...
                            // track
                            ui.painter()
                                .rect_filled(rect, 2.0, Color32::from_white_alpha(18));
                ...
                ui.label(
                    RichText::new("QUALITY ↔ PERFORMANCE")
                        .small()
                        .color(ink.muted),
                );
                ...
                ui.horizontal(|ui| {
                    ui.checkbox(&mut settings.auto_resolution, "auto fps")
                        .on_hover_text(
                            "Dynamic resolution — auto-adjust render scale to hold the target fps.",
                        );
                    ui.add_enabled(
                        settings.auto_resolution,
                        egui::DragValue::new(&mut settings.fps_target)
                            .range(20.0..=240.0)
                            .speed(1.0)
                            .suffix(" fps"),
                    );
                ...
                .on_hover_text(
                    "Render below native and upscale — the biggest fps lever at high DPI.",
                );
                ui.add(Slider::new(&mut settings.shadow_max, 0..=8).text("shadow maps"))
                    .on_hover_text(
                        "Hero (per-beam) shadow maps. Each is a depth pass (~2-3 ms at Retina).",
                    );
                ui.add(Slider::new(&mut settings.steps, 24..=176).text("raymarch steps"))
                    .on_hover_text(
                        "Volumetric beam quality. Fewer = faster, more = smoother beams.",
                    );
                ui.add(Slider::new(&mut settings.bloom, 0.0..=1.5).text("bloom"));
                ui.checkbox(&mut settings.froxel_volumetric, "froxel fog (mass beams)")
                    .on_hover_text(
                        "Compute fog grid for wide washes — faster on huge rigs, coarser beams.",
                    );
            }
        });
}
