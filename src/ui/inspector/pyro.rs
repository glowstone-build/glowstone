...
impl Inspect for PyroDevice {
    fn inspect(&mut self, p: &mut Props) {
        ...
        p.group("Effect", icon::PYRO, true, |p| {
            if spark {
                p.f32("Height", &mut self.height_m)
                    .range(0.5..=8.0)
                    .suffix(" m")
                    .slider()
                    .no_clamp();
                p.f32("Amount", &mut self.density)
                    .range(0.0..=1.0)
                    .slider()
                    .no_clamp();
                p.f32("Spread", &mut self.cone_deg)
                    .range(0.0..=45.0)
                    .suffix("°")
                    .slider()
                    .no_clamp();
                p.f32("Brightness", &mut self.brightness)
                    .range(1.0..=20.0)
                    .slider()
                    .no_clamp();
                p.f32("Hot colour", &mut self.color_t0_k)
                    .range(1500.0..=3500.0)
                    .suffix(" K")
                    .slider()
                    .no_clamp();
                p.f32("Tip colour", &mut self.color_t1_k)
                    .range(900.0..=1600.0)
                    .suffix(" K")
                    .slider()
                    .no_clamp();
                ...
                p.f32("Spread", &mut self.cone_deg)
                    ...
                    .slider()
                    .no_clamp();
                p.f32("Opacity", &mut self.opacity)
                    .range(0.05..=1.0)
                    .slider()
                    .no_clamp();
        ...
        p.group("Quality", icon::PERF, true, |p| {
            ...
            p.custom("Max particles", true, |ui| {
                ui.add(
                    egui::DragValue::new(&mut self.max_particles)
                        .speed(50.0)
                        .range(1..=u32::MAX),
                );
            });
        ...
        p.group("DMX", icon::PATCH, true, |p| {
            ...
            if let Some(patch) = &mut self.patch {
                p.custom("Universe", true, |ui| {
                    ...
                    if ui
                        .add(egui::DragValue::new(&mut v).speed(1.0).range(1..=63999))
                        .changed()
                    {
                ...
                p.custom("Address", true, |ui| {
                    ...
                    if ui
                        .add(egui::DragValue::new(&mut a).speed(1.0).range(1..=512))
                        .changed()
                    {
