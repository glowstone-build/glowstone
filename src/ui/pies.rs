...
fn shading_pie_choices() -> Vec<pie::Choice<ShadingChoice>> {
    ...
    vec![
        pie::Choice::new(
            icon::VIEWPORT,
            "Beauty",
            ShadingChoice::Mode(ViewportMode::Beauty),
        ),
        pie::Choice::new(icon::PATCH, "Grid", ShadingChoice::ToggleGrid),
        pie::Choice::new(
            icon::FIXTURE,
            "Unlit",
            ShadingChoice::Mode(ViewportMode::Unlit),
        ),
        pie::Choice::new(icon::PERF, "Stats", ShadingChoice::ToggleStats),
        pie::Choice::new(
            icon::GEOMETRY,
            "Wireframe",
            ShadingChoice::Mode(ViewportMode::Wireframe),
        ),
    ]
}
...
impl Ui {
    ...
    pub(super) fn view_pie(
        ...
        let sectors: Vec<pie::PieItem> = items
            .iter()
            .map(|(ic, lbl, _)| pie::PieItem::new(ic, *lbl))
            .collect();
        let accent = theme::accent(&self.prefs);
        if let Some(i) = pie::Pie::new(&sectors)
            .accent(accent)
            .show(ctx, &mut self.view_pie)
        {
            match items[i].2 {
...
mod tests {
    ...
    fn shading_pie_sectors_map_to_modes_and_toggles() {
        ...
        assert_eq!(
            modes,
            ViewportMode::ALL.to_vec(),
            "all three modes, once each"
        );
        // Both quick toggles are present.
        assert!(
            choices.iter().any(|c| c.value == ShadingChoice::ToggleGrid),
            "Grid toggle present"
        );
        assert!(
            choices
                .iter()
                .any(|c| c.value == ShadingChoice::ToggleStats),
            "Stats toggle present"
        );
        // No sector is unlabelled.
        assert!(
            choices.iter().all(|c| !c.label.is_empty()),
            "every sector is labelled"
        );
        ...
        assert_eq!(
            ...
            "Mode sector switches display mode"
        );

        let grid0 = ui.settings.show_grid;
        ui.apply_shading_choice(ShadingChoice::ToggleGrid);
        assert_ne!(
            ui.settings.show_grid, grid0,
            "Grid sector flips the grid flag"
        );

        let stats0 = ui.prefs.show_stats;
        ui.apply_shading_choice(ShadingChoice::ToggleStats);
        assert_ne!(
            ui.prefs.show_stats, stats0,
            "Stats sector flips the stats overlay flag"
        );
    }
